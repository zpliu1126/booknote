...
+ 在测序数据中`06_Alignment/all.collapsed.gff`文件中有每条isform的信息
  ...
  ```bash
  sed 's/gene_id [^t]*transcript_id//g' all.collapsed.gff |awk '$3~/^t/{isformLen[$9]=$5-$4+1;exonCount[$9]=0}$3~/^e/{exonCount[$9]+=1}END{for(i in isformLen){printf i"\t"isformLen[i]"\t"exonCount[i]"\n"}}'
  ```
...
+ 参考基因组gff文件中有每个mRNA的注释信息
  ...
  ```bash
  awk -F ";" '{print $1}' Ghirsutum_gene_model.gff3|sed 's/\.exon.*//g' |awk '$3~/^m/{isformLen[$9]=$5-$4+1;exonCount[$9]=0}$3~/^e/{exonCount[$9]+=1}END{for(i in isformLen){printf i"\t"isformLen[i]"\t"exonCount[i]"\n"}}'
  ```
