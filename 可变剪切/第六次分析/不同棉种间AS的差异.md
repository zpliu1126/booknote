...
> AS有助于形成基因组特异性的转录本，接下来分析At、Dt中不存在保守转录本的这些基因的特异性的转录本是否与AS有关；做个比例，找些特例出来。
>
...
A2与D5中30%的基因没有保守的转录本而在At与Dt中存在60%多的同源基因没有保守的转录本，这很矛盾。接下来通过将At、Dt中不存在保守转录本的同源基因分别与各自的二倍体祖先基因进行比较，分析这些同源基因特异性的转录本是否存在功能

> Ghir_A01G000230 基因与DGhir_D01G000240 就存在两个不一样的转录本；并且都高表达；而在二倍体中都只转录出与D基因组相同的转录本
>
...
>
> 这里讨论AS对于基因转录本分化的影响，例如Dt基因组有一个与其他三个基因组都不一样的转录本，通过分析这个转录本与参考转录本是一致的；而这个参考转录本与其他转录本相比发生了一个AS事件，这就可以说明AS导致同源基因转录本的差异
>
> 根据At、Dt差异的转录本，和四个同源基因的情况进行分类：
>
> 1. A2、D5、At中保守；Dt不保守(这个不保守的转录本有两种命运，与参考转录本CDS一致，与参考转录本CDS不一致)
> 2. A2、D5、Dt中保守；At不保守
> 3. A2、D5中同样不保守；但是A2和At保守、D5和Dt保守
>    1. A2中同时检测到At、Dt，多倍化后发生分化
> 4. D5中同时检测到At、Dt，多倍化后发生分化
