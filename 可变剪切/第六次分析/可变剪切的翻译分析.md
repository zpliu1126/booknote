...
do
##other类型
awk '$NF=="Annotion"{print $0}' 11|grep noreference |wc -l
awk '$4==""{print $0}' ORF.txt |wc -l
##没有改变编码框的前提下，改变了氨基酸序列
awk '$NF=="Annotion"{print $0}' 11|grep inframeChange |wc -l
awk '$NF=="Annotion"{print $0}' 11|awk '$9=="noframeshift"&&($4!=$7||$5!=$8){print $0}' |wc -l
##ORF没有改变
awk '$NF=="Annotion"{print $0}' 11|awk '$9=="noframeshift"&&$4==$7&&$5==$8{print $0}' |wc -l
##改变了编码框
awk '$NF=="Annotion"{print $0}' 11|awk '$9=="frameshift"{print $0}'  |wc -l
done
```

| 转录本类型     | inframeChange | frameshift | unchange ORF | Other |
| -------------- | ------------- | ---------- | ------------ | ----- |
| TM1_Annotion   | 8802          | 9201       | 25816        | 2665  |
| TM1_unAnnotion | 10228         | 12248      | 4266         | 10569 |
| A2_Annotion    | 4454          | 5873       | 15484        | 3004  |
| A2_unAnnotion  | 9771          | 11445      | 7918         | 9571  |
| D5_Annotion    | 6764          | 6925       | 19872        | 1487  |
| D5_unAnnotion  | 5251          | 5830       | 2706         | 3387  |
...
do
##终止密码子没变，ORF改变了
awk '$(NF-1)=="Annotion"{print $0}' ORF_Annotion.txt |grep inframeChange |wc -l
awk '$(NF-1)=="Annotion"{print $0}' ORF_Annotion.txt |awk '$4!=$7&&$5==$8{print $0}'|wc -l
##提前终止的密码子
awk '$(NF-1)=="Annotion"{print $0}' ORF_Annotion.txt |awk '$10=="earlyStop"{print $0}'  |wc -l
##滞后的终止密码子
awk '$(NF-1)=="Annotion"{print $0}' ORF_Annotion.txt |awk '$11=="laterStop"{print $0}'  |wc -l
##终止密码子和ORF都没有改变
awk '$(NF-1)=="Annotion"{print $0}' ORF_Annotion.txt |awk '$9=="noframeshift"&&$4==$7&&$5==$8{print $0}' |wc -l
##Other类型
awk '$(NF-1)=="Annotion"{print $0}' ORF_Annotion.txt|grep noreference |wc -l
awk '$4==""{print $0}' ORF.txt |wc -l
done
```
...
| 转录本类型    | 终止密码子没变ORF改变了 | 提前终止密码子 | 滞后终止的密码子 | 终止密码子和ORF都没改变 | Other类型 |
| ------------- | ----------------------- | -------------- | ---------------- | ----------------------- | --------- |
| TM_Annotion   | 9426                    | 7143           | 1434             | 25816                   | 2665      |
| TM_unAnnotion | 12699                   | 8800           | 977              | 4266                    | 10166     |
| A2_Annotion   | 4722                    | 5579           | 26               | 15484                   | 3004      |
| A2_unAnnotion | 11650                   | 9100           | 466              | 7918                    | 9164      |
| D5_Annotion   | 6577                    | 6201           | 911              | 19872                   | 1229      |
| D5_unAnnotion | 6189                    | 4275           | 617              | 2706                    | 3129      |
...
#### 分析四种主要AS的转录本发生in-frame change比例

| 基因组 | IR       | ES       | AA       | AD       |
| ------ | -------- | -------- | -------- | -------- |
| TM-1   | 0.399232 | 0.52504  | 0.485449 | 0.455526 |
| A2     | 0.352579 | 0.480861 | 0.452474 | 0.426698 |
| D5     | 0.477185 | 0.645914 | 0.634221 | 0.601886 |



#### 分析四种AS的转录本发生early stop的比例

| 基因组 | IR       | ES       | AA       | AD       |
| ------ | -------- | -------- | -------- | -------- |
| TM-1   | 0.329611 | 0.208939 | 0.232761 | 0.243133 |
| A2     | 0.344982 | 0.249601 | 0.253307 | 0.261025 |
| D5     | 0.36602  | 0.207782 | 0.230942 | 0.235249 |

##### 存在AS的转录本与不存在AS的转录本表达水平是否存在差异

> 有些由于AS导致编码框提前终止，导致转录本表达水平下降；可能起着调控基因表达的作用
>
> 有些由于AS导致编码框内发生了改变，但是对于的转录本水平可能更加高、或者就没有变化可能对应着新的功能
>
> 找出这些AS事件和对应的转录本与基因；在不同棉种之间讲这个故事

### 进一步根据AS的数据、ORF的数据对转录本进行分类

+ 发生了AS，但是导致了编码框的提前终止、
+ 发生AS；同时发生了inframe-change；例如增加的某一段序列会被翻译但是没有诱发提前终止

```bash
##对发生AS的转录本，整合它的Annotion、ORF信息
python ~/work/Alternative/result/Gh_result/CO31_32_result/ORF/AddAnnotionTag.py ../AS2/Alternative_isoform.txt ORF_Annotion.txt  11
mv 11 ORF_Annotion.txt
###统计数据
for i in 1
do
##other类型
awk '$NF=="AS"{print $0}' ORF_Annotion.txt|grep noreference |wc -l
awk '$4==""{print $0}' ORF_Annotion.txt |wc -l
##没有改变编码框的前提下，改变了氨基酸序列
awk '$NF=="AS"{print $0}' ORF_Annotion.txt|grep inframeChange |wc -l
awk '$NF=="AS"{print $0}' ORF_Annotion.txt|awk '$9=="noframeshift"&&($4!=$7||$5!=$8){print $0}' |wc -l
##ORF没有改变
awk '$NF=="AS"{print $0}' ORF_Annotion.txt|awk '$9=="noframeshift"&&$4==$7&&$5==$8{print $0}' |wc -l
##改变了编码框
awk '$NF=="AS"{print $0}' ORF_Annotion.txt|awk '$9=="frameshift"{print $0}'  |wc -l
done
```
